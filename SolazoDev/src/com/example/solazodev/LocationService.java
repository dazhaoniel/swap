...
public class LocationService extends Service implements LocationListener {
    ...
    protected Solazo solazo;
    private static final float MINIMUM_ACCURACY_METERS = 5000;	// 5 km
    private static final long MINIMUM_AGE = 15*60*1000;		// 15 minutes
...
//        mSolazo.setCurrentLocation(location);
    ...
    public boolean refreshCurrentLocation(Solazo solazo) {
        if (manager != null) {
            Location existingLoc = solazo.getLocation();
            List<String> providerList = manager.getAllProviders();
            for (String provider : providerList) {
                Log.d(LOGTAG, "Checking location provider: \"" + provider + "\"");
                Location loc = manager.getLastKnownLocation(provider);
                if (loc != null) {
                    long tAge = System.currentTimeMillis() - loc.getTime();
                    Log.d(LOGTAG, "Location provider \"" + loc.getProvider() + "\": age=" + tAge + " ms, accuracy=" + loc.getAccuracy() + " m)");
                    existingLoc = getBetterLocation(loc, existingLoc);
                }
            }

            if (existingLoc == null) {
                Log.d(LOGTAG, "No location provider has acceptable data, and no existing location");
                return false;
            }
            long tAge = System.currentTimeMillis() - existingLoc.getTime();
            if (tAge < MINIMUM_AGE && existingLoc.getAccuracy() < MINIMUM_ACCURACY_METERS) {
                solazo.setCurrentLocation(existingLoc);
                Log.d(LOGTAG, "Updated location (via \"" + existingLoc.getProvider() + "\"; age=" + tAge + " ms, accuracy=" + existingLoc.getAccuracy() + " m)");
                return true;
            } else {
                Log.d(LOGTAG, "Location provider \"" + existingLoc.getProvider() + "\" not acceptable: age=" + tAge + " ms, accuracy=" + existingLoc.getAccuracy() + " m");
            }
